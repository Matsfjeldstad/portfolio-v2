...
export function HamburgerMenu({ setNavIsOpen, navIsOpen, navColor }: Hamburger) {
    return (
        ...
        >
            ...
            <motion.div
                {...(navIsOpen && {
                    animate: { rotate: -45, margin: 0, y: -8 },
                })}
                className={classNames("z-20 h-1 w-7 rounded", `bg-${navColor}`)}
            ></motion.div>
        </motion.div>
...
function NavLinks({ navIsOpen, setNavIsOpen }: Navlinks) {
                    ...
                    >
                        <div
                            onClick={() => {
                                ...
                                scrollToId(link.scrollId, router);
                            }}
...
export default function Navbar() {
    ...
    const setNavColorBasedOnCondition = useNavColorStore(
        (state) => state.setNavColorBasedOnCondition
    );
