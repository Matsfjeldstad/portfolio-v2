...
export default function HeroBanner({}: Props) {
  ...
  const gradientOpacity = useTransform(scrollYProgress, [0.5, 1], [0.5, 0]);
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="pointer-events-none fixed inset-0 z-0 h-full w-screen bg-[radial-gradient(at_bottom_left,_#B12E82_10%,_transparent_70%)] "
      />
      <motion.div style={{ opacity, scale, position }} className="top-40 flex max-w-3xl flex-col items-center">
        <p>case study into</p>
        <ExtraHexLogo fill="white" className="h-24 w-fit" />
        <p className="capitalize text-gray-400"> Alternative front end for erc-20 and prc-20 contract HEX</p>
        <div className="relative mt-10">
          ...
          <ArrowDown className="absolute left-1/2 top-1/2 h-10 w-10 translate-x-[-50%] translate-y-[-50%]" />
        </div>
      </motion.div>
    </motion.section>
  );
